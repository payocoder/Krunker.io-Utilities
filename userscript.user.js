// ==UserScript==
// @name                KrunkerPlus Menu by Unkn0wn Cheats
// @author              Unkn0wn Cheats
// @version             1.8.8
// @description         A cheat created by Unkn0wn Cheats
...
// @grant               none
// ==/UserScript==
alert("Shoutout to Storm Wolf! Thank You! Script Created and scripted by Unk0wn cheats")
...
class Utilities {
    ...
    onLoad() {
        this.menus
        .set('Unkn0wn Cheats Menu', [this.newFeature('Self', []), this.newFeature('Weapon', []), this.newFeature('Visual', []), this.newFeature('Settings', [])])
        .set('Self', [this.newFeature('AutoBhop', ['Off', 'Auto Jump', 'Key Jump', 'Auto Slide', 'Key Slide']), this.newFeature('Unkn0wnCheatsSettings', ['Off', 'On'])])
        ...
        for (let i = 0, sz = this.features.length; i < sz; i++) {
            ...
            switch (feature.name) {
                ...
                case 'AutoBhop':
                    ...
                    break;
                case 'Unkn0wnCheatsSettings':
                    if (feature.value) new Map([ ["fov", 100], ["fpsFOV", 100], ["weaponBob", 1], ["weaponLean", 6], ["weaponOffX", 2], ["weaponOffY", 2], ["weaponOffZ", 2] ]).forEach(function(value, key, map) { window.setSetting(key, value) });
                    break;
                case 'UseDeltaForce':
    ...
    resetSettings() {
        if (confirm("Are you sure you want to reset all your Unkn0wn Cheats settings? This will also refresh the page")) {
    ...
    drawMenu() {
        if (this.settings.showMenu) {
            switch(this.activeMenuIndex) {
                case 0: this.drawMenuItem('Unkn0wn Cheats Menu'); break;
...
function patchGame(source) {
    ...
    for (const [name, item] of patches) {
        ...
        if (source === patched) {
            alert(`Error while patching ${name}`);
            continue;
...
alert("[Injected.] Refresh page until you see message in chat")
document.title = "Krunker.io [Unkn0wn Cheats]";
document.getElementById("instructions").style.color = "Green";
document.getElementById('instructions').innerHTML = 'Unkn0wn Cheats';
document.getElementById('modVote').innerHTML = 'Unkn0wn Cheats';
document.getElementById("texts3DHolder").innerHTML = 'Use arrow to move in menu [Rigth to activate]';
document.getElementById("krunkerio_728x90_1").remove();
var d = document.createElement('div');
d.style.cssText = 'width:8px;height:8px;background-color:#FFFFFF;position:absolute;margin:auto;top:0;right:0;bottom:0;left:0;z-index:200;border-radius:4px';
document.body.appendChild(d);
document.getElementById('chatList').innerHTML = '<div class="chatItem" style="word-break:break-all;overflow-wrap:break-word;"><span class="chatMsg"><span style="color:#B22222">Developer</span><img style="opacity:2.0;margin-right:9px;" <span style="color:#FFD700">Injected</span><div class="chatItem" style="word-break:break-all;overflow-wrap:break-word;"><span class="chatMsg"><img style="opacity:0.7;margin-right:9px;"<span style="color: #000000">Use arrows to move in menu.</span></span></span></div></div>';
